...
generate_rpm_package()
{
	...
	cp -avf ${SOURCE_DIR}/scripts/aznfs_install_github_to_pmc.sh ${STG_DIR}/${rpm_dir}/tmp${rpm_buildroot_dir}/${rpm_pkg_dir}${opt_dir}/
...
generate_tarball_package() {
    ...
    sed -i -e 's/AKS_USER="false"/AKS_USER="true"/' -e "s/RELEASE_NUMBER_FOR_AKS=x.y.z/RELEASE_NUMBER_FOR_AKS=${RELEASE_NUMBER}/" ${STG_DIR}/tarball/${tar_pkg_dir}/usr/sbin/aznfswatchdog
...
cp -avf ${SOURCE_DIR}/scripts/aznfs_install_github_to_pmc.sh ${STG_DIR}/deb/${pkg_dir}${opt_dir}/
