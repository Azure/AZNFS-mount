...
compare_CA_thumbprint()
{
    ...
    local expected_thumbprint=$(awk '/DigiCert Global Root G2/ {found=1} found && /BEGIN CERTIFICATE/,/END CERTIFICATE/ {print} found && /END CERTIFICATE/ {exit}' /etc/pki/ca-trust/extracted/pem/tls-ca-bundle.pem | openssl x509 -noout -fingerprint -sha1 | cut -d'=' -f2)

    if [ "$thumbprint" != "$expected_thumbprint" ]; then
        eecho "[FATAL] Thumbprint of the installed DigiCert Global Root G2 certificate does not match the expected value!"
        return 1
    fi
}
