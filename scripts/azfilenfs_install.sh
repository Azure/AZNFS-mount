...
ensure_pkg()
{
    ...
    if [ "$distro" == "ubuntu" ]; then
        ...
        apt=1
        apt install -y $pkg
    elif [ "$distro" == "centos" -o "$distro" == "rhel" ]; then
...
if [ $apt -eq 1 ]; then
    ...
    # TODO: Test this on suse
    install_cmd="zypper"
    zypper -y install nfs-client
    install_error=$?
else
    # TODO: Test this on CentOS
    install_cmd="yum"
...
if [ $install_error -ne 0 ]; then
    eecho "[FATAL] Error installing nfs package (Error: $install_error). See '$install_cmd' command logs for more information."
    exit 1
fi
...
if [ $install_error -ne 0 ]; then
    eecho "[FATAL] Error installing stunnel (Error: $install_error). See '$install_cmd' command logs for more information."
    exit 1
fi

# Create directory under /etc/stunnel to store the server certificate
mkdir -p /etc/stunnel/microsoft/aznfs/nfsv4_fileShare/certs

# TODO: Copy/download the server certificate. Run below command to download the server certificate.
# wget <location of server certificate> -P /etc/stunnel/microsoft/aznfs/nfsv4_fileShare/certs/
