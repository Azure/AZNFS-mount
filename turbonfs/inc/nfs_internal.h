...
struct mount_options
{
    ...
    mount_options():
        ...
        authtype(aznfsc_cfg.auth ? "AzAuthAAD" : "AzAuthNone")
    {
        assert(!server.empty());
    ...
    const std::string get_url_str() const
    {
        ...
        if (nfs_port == 2048 || nfs_port == 2047) {
            size = std::snprintf(
                                ...
                                url.size(),
                                "nfs://%s%s/?version=3&debug=%d&dircache=0&nfsport=%d&mountport=%d&timeo=%d&retrans=%d&rsize=%d&wsize=%d&readdir-buffer=%d",
                                server.c_str(),
                                export_path.c_str(),
                                debug,
                                nfs_port,
            ...
            size = std::snprintf(
                                const_cast<char*>(url.data()),
                                url.size(),
                                "nfs://%s%s/?version=3&debug=%d&dircache=0&timeo=%d&retrans=%d&rsize=%d&wsize=%d&readdir-buffer=%d",
                                server.c_str(),
                                export_path.c_str(),
                                debug,
                                timeo,
