...
- Mount the Azure File NFSv4 share using following command:
	...
	```
   If using isolated environments, or using custom DNS in any cloud other that the public cloud, ensure that the environment variable "AZURE_ENDPOINT_OVERRIDE" is set to the appropriate endpoint before running the mount command. For example:
	```
	export AZURE_ENDPOINT_OVERRIDE="core.chinacloudapi.cn"
	```
